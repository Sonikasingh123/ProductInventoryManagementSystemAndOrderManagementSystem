...
export default function ProductDetailPage({ id, onBack }: any) {
  ...
  const handleStockAdjust = async () => {
    ...
    const trimmedAmount = stockAmount.trim();

    // Empty input
    if (!trimmedAmount) {
      setStockError('Enter a stock adjustment amount.');
      return;
    }

    const amount = Number(trimmedAmount);

    // Must be a whole number and cannot be zero
    if (!Number.isInteger(amount) || amount === 0) {
      setStockError('Enter a non-zero whole number.');
      return;
    }

    // Prevent extremely large adjustments from the UI
    if (Math.abs(amount) > 999999) {
      setStockError('Stock adjustment must be between -999999 and 999999.');
      return;
    }

    try {
      const response = await fetch(
        `/api/products/${product.id}/stock?amount=${amount}`,
        {
          method: 'PATCH',
        }
      );
      ...
      if (!response.ok) {
        setStockError(
          data?.message || data?.error || 'Unable to adjust stock.'
        );
        return;
      }
      ...
      setStockError('Unable to adjust stock. Please try again.');
    }
  };
